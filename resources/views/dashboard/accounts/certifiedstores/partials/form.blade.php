...
@if(Settings::get('dashboard_template') == 'syndron')
    <div class="mb-3">
        ...
        <select class="single-select" value="{{isset($certifiedstore) ? $certifiedstore->country_id : ""}}" name="country_id">
...
<select2
    name="country_id"
    id="country_id"
    value="{{ isset($certifiedstore) ? $certifiedstore->country_id : "" }}"
...
@isset($certifiedstore)
    {{ BsForm::image('avatar')->collection('avatars')->files($certifiedstore->getMediaResource('avatars')) }}
@else
