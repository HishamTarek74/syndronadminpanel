...
@if(Settings::get('dashboard_template') == 'syndron')
    <div class="mb-3">
        ...
        <select class="single-select" value="{{isset($store) ? $store->country_id : ""}}" name="country_id">
...
<select2
    name="country_id"
    id="country_id"
    value="{{ isset($store) ? $store->country_id : "" }}"
...
@isset($store)
    {{ BsForm::image('avatar')->collection('avatars')->files($store->getMediaResource('avatars')) }}
@else
